...
const CategoryComponent = ({ shoes,auth,product }) => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {shoes.map((shoe, index) => (
                    <div key={index} className="bg-white p-4 shadow-md rounded-lg">
                        <img src={shoe.image_url} alt={shoe.name} className="w-full mb-4" />
                        <div className="text-lg font-semibold mb-2">{shoe.name}</div>
                        <div className="text-gray-600 mb-2">${shoe.price}</div>
                        <div className="text-gray-500">{shoe.description}</div>

  ...
  const CategoryComponent = ({ women ,auth,product }) => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {women.map((women, index) => (
                      <div key={index} className="bg-white p-4 shadow-md rounded-lg">
                          <img src={women.image_url} alt={women.name} className="w-full mb-4" />
                          <div className="text-lg font-semibold mb-2">{women.name}</div>
                          <div className="text-gray-600 mb-2">${women.price}</div>
                          <div className="text-gray-500">{women.description}</div>
